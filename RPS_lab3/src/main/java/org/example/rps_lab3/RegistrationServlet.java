        ...
        try {
            ...
            try{
                ...
                if(!(authResult == 0)) {
                    ...
                    out.print(jsonResponse.toString()); //формируем ответ -2, если юзер существует
                } else {
                    int regResult = DBConnect.registrate_user(username, password);

                    if (regResult > 0) {
                        //Сохранение id в сессию
                        HttpSession session = request.getSession();
                        session.setAttribute("userId", regResult);
                        session.setAttribute("username", username);

                        //Сохранение id в куки
                        Cookie userIdCookie = new Cookie("userId", String.valueOf(regResult));
                        userIdCookie.setMaxAge(24 * 60 * 60);
                        userIdCookie.setPath("/");
                        response.addCookie(userIdCookie);
                    }

                    jsonResponse.put("status", regResult);
